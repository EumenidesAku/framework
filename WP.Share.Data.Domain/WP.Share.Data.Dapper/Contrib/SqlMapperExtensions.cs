...
namespace WP.Share.Data.Dapper.Contrib
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        /// <summary>
        /// The function to get a a Key Alisa from a given <see cref="Type"/>
        /// </summary>
        /// <param name="type">The <see cref="Type"/> to get a table name for.</param>
        public delegate string KeyAliasMapperDelegate(Type type);
        ...
        private static readonly ConcurrentDictionary<RuntimeTypeHandle, string> TypeKeyAlias = new ConcurrentDictionary<RuntimeTypeHandle, string>();
        ...
        private static PropertyInfo GetSingleKey(Type type)
        {
            ...
            var keyCount = keys.Count + explicitKeys.Count;
            if (keyCount > 1)
                throw new DataException($"{type.Name} only supports an entity with a single [Key] or [ExplicitKey] property");
            if (keyCount == 0)
                throw new DataException($"{type.Name} only supports an entity with a [Key] or an [ExplicitKey] property");
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            {
                var keyAlias = GetKeyAlias(type);
                var name = GetTableName(type);
                if (!DefaultKey.Equals(keyAlias))
                {
                    sql = $"select {keyAlias} AS Id ,* from {name} where {keyAlias} = @id";
                }
                else
                {
                    sql = $"select * from {name} where {keyAlias} = @id";
                }
        ...
        /// Specify a custom table name mapper based on the POCO type name
        /// </summary>
        public static KeyAliasMapperDelegate KeyAliasMapper;

        private static string GetKeyAlias(Type type)
        {
            if (TypeKeyAlias.TryGetValue(type.TypeHandle, out string name)) return name;

            if (KeyAliasMapper != null)
            {
                name = KeyAliasMapper(type);
            }
            else
            {
#if NETSTANDARD1_3
                ...
                //NOTE: This as dynamic trick falls back to handle both our own Table-attribute as well as the one in EntityFramework 
                var aliasAttrName =
                    info.GetCustomAttribute<KeyAliasAttribute>(false)?.Alias
                    ?? (info.GetCustomAttributes(false).FirstOrDefault(attr => attr.GetType().Name == "KeyAliasAttribute") as dynamic)?.Alisa;

                if (aliasAttrName != null)
                {
                    name = aliasAttrName;
                }
                else
                {
                    name = GetSingleKey(type).Name;
                }
            }

            TypeKeyAlias[type.TypeHandle] = name;
            return name;
        }
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < allPropertiesExceptKeyAndComputed.Count; i++)
            {
                ...
                if (DefaultKey.Equals(property.Name))
                {
                    string aliasKey = GetKeyAlias(type);
                    adapter.AppendColumnName(sbColumnList, aliasKey);
                }
                else
                {
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < nonIdProps.Count; i++)
            {
                ...
                {
                    string aliasKey = GetKeyAlias(type);
                    adapter.AppendColumnNameEqualsId(sb, aliasKey);
                }
                else
                {
        ...
        public static bool DeleteById<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out string sql))
            {
                ...
                var alias = GetKeyAlias(type);

                sql = $"delete from {GetTableName(type)} where {alias} = @id";
            }
    ...
    public class KeyAliasAttribute : Attribute
    {
        ...
        /// Creates a table mapping to a specific name for Dapper.Contrib commands
        /// </summary>
        /// <param name="keyAlias">The name of this table key in the database.</param>
        ...
        /// <summary>
        /// The name of the table key alias in the database
        /// </summary>
...
public partial interface ISqlAdapter
{
    ...
    void AppendColumnNameEqualsId(StringBuilder sb, string columnName);
}
...
public partial class SqlServerAdapter : ISqlAdapter
{
    ...
    public void AppendColumnNameEqualsId(StringBuilder sb, string columnName)
    {
        sb.AppendFormat("[{0}] = @{1}", columnName, SqlMapperExtensions.DefaultKey);
    }
}
